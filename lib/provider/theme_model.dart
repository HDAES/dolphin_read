...
class ThemeModel with ChangeNotifier {
  ...
  ThemeModel() {
    /// 用户选择的明暗模式
    _userDarkMode = StorageUtil().getBool(kThemeUserDarkMode)??false;

    /// 获取主题色
    _themeColor = Colors.primaries[
       StorageUtil().getInt(kThemeColorIndex) ?? 5];

    /// 获取字体
    _fontIndex =  StorageUtil().getInt(kFontIndex) ?? 0;
  }
  ...
  // }
   ...
   saveTheme(bool userDarkMode, MaterialColor themeColor) async {
     var index = Colors.primaries.indexOf(themeColor);
      await Future.wait([
        StorageUtil().setBool(kThemeUserDarkMode, userDarkMode),
        StorageUtil().setInt(kThemeColorIndex, index)
      ]);
   }

   /// 字体选择持久化
  static saveFontIndex(int index) async {
    await  StorageUtil().setInt(kFontIndex, index);
  }
